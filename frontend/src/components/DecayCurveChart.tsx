...
export function DecayCurveChart({ peakViews, decayRate, days }: DecayCurveChartProps) {
  const maxDays = Math.min(Math.ceil(days), 60);
  const points: { day: number; views: number }[] = [];

  for (let d = 0; d <= maxDays; d++) {
    const views = peakViews * Math.pow(decayRate, d);
  ...
  const toY = (views: number) => {
    ...
    return padT + chartH - ((views - minViews) / (maxViews - minViews)) * chartH;
  };

  const pathD = points
    .map((p, i) => `${i === 0 ? 'M' : 'L'} ${toX(p.day).toFixed(1)} ${toY(p.views).toFixed(1)}`)
    .join(' ');

  const areaD =
    pathD +
    ` L ${toX(points[points.length - 1].day).toFixed(1)} ${(padT + chartH).toFixed(1)}` +
  ...
  return (
    ...
    >
      ...
      <div
        ...
        style={{ background: 'linear-gradient(90deg, oklch(0.68 0.26 295), oklch(0.72 0.30 340))' }}
      />
