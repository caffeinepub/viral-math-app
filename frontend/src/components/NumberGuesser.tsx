...
const questions: Question[] = [
  {
    ...
    topicEmoji: '📊',
    question: 'A reel gets 5,000 views, 300 likes, 50 comments, and 50 shares. What is the engagement rate (%)?',
    answer: 8,
    tolerance: 0.5,
    unit: '%',
  ...
  },
  {
    ...
    topic: 'Engagement Rate',
    topicEmoji: '📊',
    question: 'A reel has 20,000 views, 1,200 likes, 200 comments, and 600 shares. What is the engagement rate (%)?',
    answer: 10,
    tolerance: 0.5,
    unit: '%',
    hint: 'ER = (Likes + Comments + Shares) / Views × 100',
    explanation: '(1,200 + 200 + 600) / 20,000 × 100 = 2,000 / 20,000 × 100 = 10%',
  },
  {
    id: 3,
    topic: 'Engagement Rate',
    topicEmoji: '📊',
    question: 'Your reel has 100,000 views and a 3.5% engagement rate. How many total interactions did it get?',
    answer: 3500,
    tolerance: 1,
    unit: 'interactions',
  ...
  },
  {
    ...
    topic: 'Viral Growth',
    topicEmoji: '🚀',
    question: 'A reel starts with 50 viewers. Each shares it to 2 others. After 3 rounds, what is the total reach?',
    answer: 400,
    tolerance: 1,
    unit: 'people',
  ...
  },
  {
    ...
    topic: 'Viral Growth',
    topicEmoji: '🚀',
    question: 'Starting with 200 viewers, each sharing to 3 others over 2 rounds. What is the total reach?',
    answer: 1800,
    tolerance: 1,
    unit: 'people',
    hint: 'Reach = Initial × (Shares ^ Rounds)',
  ...
  },
  {
    ...
    topic: 'Viral Growth',
    topicEmoji: '🚀',
    question: 'A reel starts with 1,000 viewers. Each shares to 4 others. After 1 round, what is the total reach?',
    answer: 4000,
    tolerance: 1,
    unit: 'people',
    hint: 'Reach = Initial × (Shares ^ Rounds)',
  ...
  },
  {
    ...
    topic: 'View Decay',
    topicEmoji: '📉',
    question: 'A reel peaks at 10,000 views/day with a 0.5 daily decay rate. How many views on day 2?',
    answer: 5000,
    tolerance: 1,
    unit: 'views',
  ...
  },
  {
    ...
    topic: 'View Decay',
    topicEmoji: '📉',
    question: 'A reel peaks at 80,000 views with a 0.8 decay rate. How many views remain after 2 days?',
    answer: 51200,
    tolerance: 100,
    unit: 'views',
    hint: 'Views = Peak × (Decay Rate ^ Days)',
  ...
  },
  {
    ...
    topic: 'View Decay',
    topicEmoji: '📉',
    question: 'A reel peaks at 50,000 views with a 0.9 decay rate. How many views after 3 days? (round to nearest whole number)',
    answer: 36450,
    tolerance: 100,
    unit: 'views',
    hint: 'Views = Peak × (Decay Rate ^ Days)',
  ...
  },
  {
    ...
    topic: 'Engagement Rate',
    topicEmoji: '📊',
    question: 'Two reels: Reel A has 500 views and 50 interactions. Reel B has 10,000 views and 500 interactions. Which has a higher ER? Enter Reel A\'s ER (%).',
    answer: 10,
    tolerance: 0.5,
    unit: '%',
    hint: 'ER = Interactions / Views × 100',
    explanation: 'Reel A: 50/500×100 = 10%. Reel B: 500/10,000×100 = 5%. Reel A wins despite fewer views! 🔥',
  },
];

const topicGradients: Record<string, { gradient: string; glow: string; badge: string }> = {
  'Engagement Rate': {
    ...
    return (
      <div className="glass-card rounded-3xl p-8 text-center relative overflow-hidden">
        <div className="absolute inset-0 pointer-events-none"
          style={{ background: 'radial-gradient(ellipse at 50% 0%, oklch(0.68 0.26 295 / 0.15), transparent 70%)' }} />
        <div className="text-6xl mb-4">{emoji}</div>
        <h3 className="font-display text-2xl font-extrabold mb-2 gradient-text-purple-pink">
          Game Over!
        </h3>
        <p className="text-muted-foreground text-sm mb-6">
          You got <span className="font-bold text-foreground">{score} out of {questions.length}</span> correct
        </p>
        <div className="rounded-2xl p-4 mb-6 inline-block"
          style={{ background: 'oklch(0.68 0.26 295 / 0.15)', border: '1px solid oklch(0.68 0.26 295 / 0.30)' }}>
          <span className="font-display text-4xl font-extrabold gradient-text-purple-pink">{pct}%</span>
          <p className="text-xs text-muted-foreground mt-1">accuracy</p>
        </div>
        <p className="text-sm text-muted-foreground mb-6">
          {pct >= 80 ? 'You\'re a viral math genius! No cap 🧠' : pct >= 50 ? 'Solid effort! Keep practicing 💪' : 'Time to study the formulas bestie 📚'}
        </p>
        <button
          ...
          style={{ background: 'linear-gradient(135deg, oklch(0.68 0.26 295), oklch(0.72 0.28 340))', boxShadow: '0 4px 20px oklch(0.68 0.26 295 / 0.40)' }}
        >
    ...
    <div className="space-y-4">
      ...
      <div className="flex items-center justify-between px-1">
        <div className="flex items-center gap-2">
          ...
          <span className="font-display font-extrabold text-sm gradient-text-purple-pink">{score} / {questions.length}</span>
        </div>
        ...
        <span className="text-xs text-muted-foreground font-bold">{currentIndex + 1} / {questions.length}</span>
      </div>
      ...
      <div className="glass-card rounded-3xl p-6 relative overflow-hidden">
        ...
        <div className="flex gap-2 mb-4">
          ...
          <button
            ...
            style={{ background: styles.gradient, boxShadow: `0 4px 16px ${styles.glow}` }}
          >
            Submit
          </button>
        </div>
        ...
        {showHint && !feedback && (
          <div className="mt-2 rounded-2xl px-4 py-2.5 text-xs font-mono text-foreground/80"
          ...
          >
            ...
            <div>
              <p className="font-display font-bold text-sm mb-1"
                style={{ color: feedback === 'correct' ? 'oklch(0.78 0.22 145)' : 'oklch(0.72 0.28 340)' }}>
                {feedback === 'correct' ? '🔥 Correct!' : `😬 Not quite — answer: ${current.answer.toLocaleString()} ${current.unit}`}
              </p>
