...
const inputDefs = [
  {
    key: "views" as keyof ReelFields,
    emoji: "👁",
    label: "Views",
    placeholder: "e.g. 100000",
  },
  {
    key: "likes" as keyof ReelFields,
    emoji: "❤️",
    label: "Likes",
    placeholder: "e.g. 4500",
  },
  {
    key: "comments" as keyof ReelFields,
    emoji: "💬",
    label: "Comments",
    placeholder: "e.g. 320",
  },
  {
    key: "shares" as keyof ReelFields,
    emoji: "🔁",
    label: "Shares",
    placeholder: "e.g. 180",
  },
...
export function CompareReels() {
  ...
  const parseFields = (f: ReelFields) => ({
    views: Number.parseFloat(f.views),
    likes: Number.parseFloat(f.likes),
    comments: Number.parseFloat(f.comments),
    shares: Number.parseFloat(f.shares),
  });
  ...
  const validate = (f: ReelFields, label: string): string | null => {
    ...
    if ([views, likes, comments, shares].some(Number.isNaN)) {
        ...
        <div key={results.key} className="space-y-4 animate-scale-in">
          ...
          <div className="grid grid-cols-2 gap-3">
            <RateDisplay
              ...
              borderColor={
                winner === "reel1"
                  ? "oklch(0.72 0.30 340 / 0.60)"
                  : "oklch(0.72 0.30 340 / 0.25)"
              }
            />
            <RateDisplay
              label="Reel 2"
              ...
              borderColor={
                winner === "reel2"
                  ? "oklch(0.72 0.22 220 / 0.60)"
                  : "oklch(0.72 0.22 220 / 0.25)"
              }
            />
          </div>
