...
export function GrowthCurveChart({
  ...
  for (let i = 0; i <= maxRounds; i++) {
    const reach = initialViewers * sharesPerPerson ** i;
  ...
  const toY = (reach: number) => {
    ...
    return (
      padT + chartH - ((reach - minReach) / (maxReach - minReach)) * chartH
    );
  };
  ...
  const pathD = points
    .map(
      (p, i) =>
        `${i === 0 ? "M" : "L"} ${toX(p.round).toFixed(1)} ${toY(p.reach).toFixed(1)}`,
    )
    .join(" ");

  // Area fill path
  const areaD = `${pathD} L ${toX(points[points.length - 1].round).toFixed(1)} ${(padT + chartH).toFixed(1)} L ${toX(0).toFixed(1)} ${(padT + chartH).toFixed(1)} Z`;
  ...
  return (
    ...
    >
      ...
      <div
        ...
        style={{
          background:
            "linear-gradient(90deg, oklch(0.82 0.20 195), oklch(0.78 0.22 145))",
        }}
      />
      ...
      <svg
        ...
        style={{ display: "block" }}
        role="img"
        aria-label="Exponential growth curve chart"
      >
        <defs>
